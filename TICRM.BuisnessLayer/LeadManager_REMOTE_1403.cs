    ...
    public class LeadManager : BaseManager
    {
        ...
        public List<LeadDto> GetLeads()
        {
            try
            {
                ...
                List<Lead> leads = dbEnt.Leads.Include(l => l.Address).Include(l => l.Industry).Include(l => l.LeadSource).Include(l => l.LeadType).Include(l => l.Status).Include(l => l.Team).Include(l => l.User).Where(a => a.IsDeleted.ToString() != "True").ToList();
            ...
            try
            {
                ...
                if (isEditMode)
                {
                    ...
                    if (isDeleteMode)
                    {
                        ...
                        //dbEnt.Leads.Remove(leadDelete); // remove object in database
