...
namespace TICRM.Controllers
{
 ...
 ||  Author:  [Undefined]
 ||
 ||  Purpose:  [The class serves all the functionlities related with Account signIn, SignOut,Authentication etc]
 ||
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            try
            {
                ...
                LoggerManager.Info("User LoggedIn Successflly");
                ...
                switch (result)
                {
                    ...
                    case SignInStatus.LockedOut:
                        LoggerManager.Info("User LockedOut");
                        return View("Lockout");
                    case SignInStatus.RequiresVerification:
                        LoggerManager.Info("User Requires Verification");
                        return RedirectToAction("SendCode", new { ReturnUrl = returnUrl, RememberMe = model.RememberMe });
                    case SignInStatus.Failure:
                    default:
                        LoggerManager.Info("Invalid Login Attempt");
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    var user = new ApplicationUser { UserName = model.Email, Email = model.Email, LockoutEndDateUtc = DateTime.Now };
        ...
        public ActionResult LogOff()
        {
            try
            {
                ...
                LoggerManager.Info("User Logged Out and redirected to Login");
                return RedirectToAction("Index", "Dashboard");
            }catch(Exception ex)
            {
                throw ex;
